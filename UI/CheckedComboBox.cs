    ...
    public class CheckedComboBox : ComboBox {
        ...
        internal class Dropdown : Form {
            ...
            private void cclb_ItemCheck(object sender, ItemCheckEventArgs e) {
                ...
                if (e.Index != 0)                     
                {
                    SaveIndex.Add(e.Index);
                    if (cclb.GetItemChecked(0) == true)
                    {
                        int y = -1;
                        y = cclb.FindString("ALL");
                        if (y != -1)
                        {
                            cclb.SetItemChecked(0, false);
        ...
        private void DoDropDown() {
            if (!dropdown.Visible) {
                ...
                string height = ((dropdown.List.ItemHeight) * count + 2).ToString();

                if (int.Parse(height) > 500)
                {
                    dropdown.Size = new Size(this.Size.Width, 500);   //override the other size 
                }
                else
                {

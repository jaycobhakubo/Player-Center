      ...
      //  private clsUSB mobjUSB = null;
        ...
        public MCPPlayerManagementForm(PlayerManager parent)
        {
            try
            {
                ...
                GetPlayerTierDatam.getPlayertierData(0);

                //lets get the staffID
                ModuleComm getstaffID = new ModuleComm();
                int staffid = getstaffID.GetStaffId();
                int countSubItem = (menuStrip1.Items[1] as ToolStripMenuItem).DropDownItems.Count;//count how many subitem in View

                GetStaffModuleFeaturesMessage SendMessage = new GetStaffModuleFeaturesMessage(staffid, (int)EliteModule.PlayerCenter, 0);
                SendMessage.Send();
               // int ModuleFeatureID = 0;
                int IsModuleFeatureExists = 0;
                foreach (int ModuleFeatureID in SendMessage.ModuleFeatureList)
                {//since there only one right now
                    if (ModuleFeatureID == 38)
                    {
                        IsModuleFeatureExists = 1;
                        (menuStrip1.Items[1] as ToolStripMenuItem).DropDownItems[2].Visible = true;
                        break;    //if I found it - > get out now dont waste memory.              
                    }                
                }

                if (IsModuleFeatureExists == 0)//it means it doesnt exists
                {
                    (menuStrip1.Items[1] as ToolStripMenuItem).DropDownItems[2].Visible = false;
                }
...
//            m_player.MagneticCardNumber = m_magCardNum.Text; // FIX: DE6690
        ...
        private void loyalityGroupBox_Enter(object sender, EventArgs e)
        {

        }


        //private void playerLoyaltyToolStripMenuItem_Click_1(object sender, EventArgs e)
        //{
